...
import { UpdateProductByAdmin } from '../../ServicesAPI';
...
const UpdateProductyAdmin = ({EditData}) => {
    ...
    setImage(prev => ({...prev,image : image}));
    }
    ...
    useEffect(() => {
        reset({
            ...
            qualnity :EditData?.qualnity || '',
            category : EditData?.category || '',
            brand : EditData?.brand || '',
        })
        setImage({
            thumb :EditData.thumb || '',
            image : EditData?.images
        })
        setpayload({description : typeof EditData?.description === 'object' ? EditData?.description?.join(', ') : EditData?.description})
    },[EditData])
    ...
    const handleUpdateProduct = async(data) => {     
       ...
       if(invalids === 0) {   
          if(data.category) data.category = dataCate?.categories?.find(item => item.title === data.category)?.title;
          const pendingData = ({...data,...payload});
          pendingData.thumb = data?.thumb?.length === 0 ? picture?.thumb : data.thumb[0]
          const formData = new FormData();
          for(let i of Object.entries(pendingData))  formData.append(i[0],i[1]);
          pendingData.images = data?.images?.length === 0 ? picture.image : data.images
          for(let i of  pendingData.images) formData.append('images',i);

          setLoading(true)
          const template = await UpdateProductByAdmin(formData,EditData?._id);         
         ...
         <form onSubmit={handleSubmit(handleUpdateProduct)}>
           ...
           <div className='d-flex justify-content-between align-items-center mb-4 mt-4'>
            <SelectForm
              ...
              width='w-40'
              options={dataCate.categories?.map(item => ({id: item.title,value : item.title}))}
            />
            <SelectForm
              label='Brand'
              id='brand'
              ...
              defaultValue=''
              width='w-50'
              options={dataCate?.categories?.find(item =>item.title ===EditData?.category)?.brand?.map(ele => ({id : ele,value : ele}))}
            />
           </div>
           <EditorMarkdown 
              label='Infomation'
           ...
           <div className='d-flex gap-3'>
             {!_.isEmpty(picture?.image) && picture?.image?.map((item,index) => <div className='w-100 images_width object-fit-cover' key={index}><img src={item} className='h-100 w-100' /></div>)}
             {/* {picture?.image.length  ===  1 && <div className='w-100 images_width object-fit-contain'><img src={picture?.image[0]} className='h-100' /></div>} */}
  
           </div>
              {errors['images'] && <div className='text-danger'>{errors['images']?.message}</div>}
          
          <input type='submit' className='btn btn-danger mt-4' value='Update Product'/>
         </form>
...
export default UpdateProductyAdmin